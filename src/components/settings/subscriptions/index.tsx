...
import { formatBytes, convertTimestampToMs, simpleDate } from "@/utils"
...
import WithdrawalDialog, { type SubscriptionWithWithdrawal } from "./withdrawalDialog"
...
export const Subscriptions = memo(() => {
	...
	const { cancelledSubs, setCancelledSubs, withdrawnSubs, setWithdrawnSubs } = useUserStore(
		useCallback(
			state => ({
				cancelledSubs: state.cancelledSubs,
				setCancelledSubs: state.setCancelledSubs,
				withdrawnSubs: state.withdrawnSubs,
				setWithdrawnSubs: state.setWithdrawnSubs
			}),
			[]
		)
	)
	const [withdrawSub, setWithdrawSub] = useState<SubscriptionWithWithdrawal | null>(null)
	...
	const namePrefill = useMemo(() => {
		...
		const fullName = [account.account.personal.firstName, account.account.personal.lastName].filter(Boolean).join(" ").trim()

		return fullName.length > 0 ? fullName : account.account.displayName || account.account.nickName || ""
	}, [account])

	const subscriptionsSorted = useMemo((): SubscriptionWithWithdrawal[] => {
		if (!account) {
			return []
		}

		return (account.account.subs as SubscriptionWithWithdrawal[])
			.slice()
			.sort((a, b) => b.startTimestamp - a.startTimestamp)
			.map(sub => {
				let next = sub

				if (cancelledSubs.includes(sub.id)) {
					next = {
						...next,
						cancelled: 1,
				...
				if (withdrawnSubs.includes(sub.id) && (next.withdrawRequested ?? 0) === 0) {
					next = {
						...next,
						withdrawRequested: Math.floor(Date.now() / 1000)
					}
				}

				return next
			})
	}, [account, cancelledSubs, withdrawnSubs])
	...
	const onWithdrawalConfirmed = useCallback(
		(uuid: string) => {
			setWithdrawnSubs(prev => (prev.includes(uuid) ? prev : [...prev, uuid]))

			if (account) {
				account.refetch()
			}
		},
		[setWithdrawnSubs, account]
	)
	...
	return (
		...
		>
				...
				{subscriptionsSorted.length === 0 && account ? (
					...
					<div className="flex flex-col w-full p-4">
						{subscriptionsSorted.map(subscription => {
							const withdrawRequested = subscription.withdrawRequested ?? 0
							const canWithdraw = subscriptionWithdrawalEligible(subscription)
								...
								>
									...
									{canWithdraw && (
										<div className="flex flex-row px-4 pb-4">
											<Button
												variant="destructive"
												onClick={() => setWithdrawSub(subscription)}
											>
												{t("settings.withdrawal.button")}
											</Button>
										</div>
									)}
									{withdrawRequested > 0 && (
										<div className="flex flex-row px-4 pb-4 items-center gap-2">
											<XCircle
												className="text-red-500"
												size={18}
											/>
											<p className="text-sm text-muted-foreground">
												{t("settings.withdrawal.statusWithdrawn", {
													date: simpleDate(withdrawRequested)
												})}
											</p>
										</div>
									)}
			...
			<WithdrawalDialog
				...
				emailPrefill={account.account.email}
				onClose={() => setWithdrawSub(null)}
				onConfirmed={onWithdrawalConfirmed}
			/>
		</div>
	)
})
